	...
	, timerRedraw(this)
{
	...
	for(unsigned i=0; i<nTrends; ++i, ++ppds){
		...
		ppds->penPlot=*(wxThePenList->FindOrCreatePen(ppds->crPlot, 1, wxBRUSHSTYLE_SOLID));
...
void COScopeCtrl::RecreateGrid()
{
	...
	wxPen solidPen = *(wxThePenList->FindOrCreatePen(m_gridColour, 1, wxBRUSHSTYLE_SOLID));
	wxString strTemp;
